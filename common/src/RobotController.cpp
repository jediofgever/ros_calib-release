...
RobotController::RobotController(/* args */)
{
    ...
    visual_tools_ptr_.reset(
        new moveit_visual_tools::MoveItVisualTools("base_link", "/moveit_visual_markers"));
}
    ...
    moveit::planning_interface::MoveGroupInterface *move_group_ptr_)
{
    ...
    bool success =
        (move_group_ptr_->plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
    ...
    // chechk if any of joints will rotate more than a reasonable amount, return a false and not
    // execute path if there is some creazy plan found
    ...
    std::vector<double> joint_roation_limits = {M_PI, 3 * M_PI_4, 3 * M_PI_4,
                                                M_PI, 2 * M_PI,   4 * M_PI};

    for (size_t i = 0; i < my_plan.trajectory_.joint_trajectory.points.size(); i++)
    {
        trajectory_msgs::JointTrajectoryPoint this_point =
            my_plan.trajectory_.joint_trajectory.points[i];
    ...
    {
        double biggest_rotation_in_this_joint =
            std::abs(max_joint_positions[i] - min_joint_positions[i]);
        if (biggest_rotation_in_this_joint > joint_roation_limits[i])
        {
            ROS_ERROR(
                "THIS PLAN WILL VIOLATE THE DEFINED JOINT LIMITS OF %d, SKIPPING THE PLAN AND NOT "
                "EXECUTING!!!",
                i);
            return false;
        }
    ...
    if (success)
    {
        ...
        returnMoveitResult = (move_group_ptr_->execute(my_plan) ==
...
{
    bool returnMoveitResult = false;
    // get current pose ob tcp, because we will calculate a linear path, which needs current and
    // target poses
    ...
    double fraction =
        move_group_ptr_->computeCartesianPath(waypoints, eef_step, jump_threshold, trajectory);
 ...
 * @return when it finishs the task
 */
bool RobotController::moveEndEffectortoGoalinToolSpace(
    ...
    tf::TransformListener *listener_)
{
    ...
    The given goal is in tool_link, this goal is transformed to base_link and then a
    cartesian/linear path is calculated
     */
    ...
    // we will do a linear plan between current pose and given target pose, so push the current pose
    // to waypoint vector
    std::vector<geometry_msgs::Pose> waypoints;
 ...
 * @return when it finishs the task
 */
bool RobotController::moveJointstoTargetPositions(
    ...
    moveit::planning_interface::MoveGroupInterface *move_group_ptr_)
{
    ...
    bool success = (move_group_ptr_->plan(move_joint_plan) ==
                    moveit::planning_interface::MoveItErrorCode::SUCCESS);
    ...
    if (success)
    {
        ...
        returnMoveitResult = (move_group_ptr_->execute(move_joint_plan) ==
                              moveit::planning_interface::MoveItErrorCode::SUCCESS);
    }
    else
    {
 ...
 * @param move_group_ptr_
 */
void RobotController::stopRobotTrajectrory(
    moveit::planning_interface::MoveGroupInterface *move_group_ptr_)
{
...
/**
 * @brief stops current execution of any trajectory but keeps memory of goal, so the execution can
 * be continued
 *
 * @param move_group_ptr_
 */
void RobotController::pauseRobotTrajectrory(
    moveit::planning_interface::MoveGroupInterface *move_group_ptr_)
{
 ...
 *
 */
void RobotController::publishRobotTrajectory(
    moveit::planning_interface::MoveGroupInterface *move_group_ptr_,
    moveit_msgs::RobotTrajectory trajectory)
{
    // Whicj joint model group's trajectory you would like to visualize ? , in our case
    // "manipulator"
    ...
    robot_trajectory::RobotTrajectoryPtr robot_trajectory(new robot_trajectory::RobotTrajectory(
        move_group_ptr_->getRobotModel(), joint_model_group_ptr_->getName()));
    ...
    for (std::size_t i = 0; i < robot_trajectory->getWayPointCount(); ++i)
    {
        ...
        const Eigen::Isometry3d &tip_pose =
            robot_trajectory->getWayPoint(i).getGlobalLinkTransform("link_6");
        ...
        visual_tools_ptr_->publishSphere(tip_pose, rviz_visual_tools::BLUE,
