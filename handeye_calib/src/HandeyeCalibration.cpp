 ...
 */
std::pair<geometry_msgs::TransformStamped, geometry_msgs::TransformStamped>
HandeyeCalibration::getTransfromPair()
{
    ...
    try
    {
        if (eye_in_hand)
        {
            listener_->lookupTransform(robot_base_frame, robot_effector_frame, ros::Time(0.0f),
                                       rob_transform);
        }
        else
        {
            listener_->lookupTransform(robot_effector_frame, robot_base_frame, ros::Time(0.0f),
                                       rob_transform);
        }
        listener_->lookupTransform(tracking_base_frame, tracking_marker_frame, ros::Time(0.0f),
                                   opt_transform);
    ...
    std::pair<geometry_msgs::TransformStamped, geometry_msgs::TransformStamped> this_pair(rob_msg,
                                                                                          opt_msg);
    return this_pair;
}
...
void HandeyeCalibration::takeSample()
{
    ...
    std::pair<geometry_msgs::TransformStamped, geometry_msgs::TransformStamped> a_sample_pair =
        getTransfromPair();
...
{
    // we need to convert std::vector<std::pair<geometry_msgs::TransformStamped,
    // geometry_msgs::TransformStamped>> TO std::pair<visp_hand2eye_calibration::TransformArray,
    // visp_hand2eye_calibration::TransformArray>
    ...
    for (size_t i = 0; i < transform_pair_samples_vector.size(); i++)
    {
        ...
        camera_marker_samples.transforms.push_back(
            transform_pair_samples_vector[i].second.transform);
    }
    std::pair<visp_hand2eye_calibration::TransformArray, visp_hand2eye_calibration::TransformArray>
        visp_samples(hand_world_samples, camera_marker_samples);
...
void HandeyeCalibration::computeCalibration(int curr_sample, int total_sample)
{
    ...
    // the  calibrate from vpHandEyeCalibration uses a vpHomogeneousMatrix , we use visp_bridge to
    // convert TransformArray to a vector of this vpHomogeneousMatrix
