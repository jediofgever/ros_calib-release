...
class HandEyeNode
{
  ...
  public:
    ...
    /**
     * @brief callback function that recieves latest transfrom of marker to camera , and stores it
     * in latest_marker_to_camera_transform
     *
     * @param msg
     */
    ...
    /**
     * @brief accepts a stamped transform, compares the accepted transfrom stamp with the current
     * time, if the transform was recieved more than 1 seconds ago, return false, if the transform
     * is fresher than 1 second , returns true, which makes this transform valid
     *
     * @param transform_msg
     * @return true
     * @return false
...
HandEyeNode::HandEyeNode(ros::NodeHandle *nh)
{
    ...
    // subscribe to "/aruco_tracker/transform" topic to get latest known transfrom from marker to
    // camera
    transfrom_marker_to_camera_sub =
        nh_->subscribe("/aruco_tracker/transform", 1, &HandEyeNode::marker2CamTransCallback, this);
    marker_pose_in_camera_link_sub =
        nh_->subscribe("/aruco_tracker/pose", 1, &HandEyeNode::markerPoseCallback, this);
...
/**
 * @brief accepts a stamped transform, compares the accepted transfrom stamp with the current time,
 * if the transform was recieved more than 1 seconds ago, return false, if the transform is fresher
 * than 1 second , returns true, which makes this transform valid
 *
 * @param transform_msg
 * @return true
 * @return false
...
bool HandEyeNode::isTransformValid()
{
    ...
    double elapsed_time_from_latest_transfrom_double =
        std::abs(elapsed_time_from_latest_transfrom.toSec());
...
void HandEyeNode::arrangeInitialPositionAtTopofMarker()
{
    ...
    double current_distance_to_initial_pose = RobotPoseGenerator::getDistanceBetweenPose(
        initial_pose, move_group_ptr_->getCurrentPose().pose);
    ...
    if (is_eye_in_hand)
    {
        ...
        robot_contoller_ptr_->moveEndEffectortoGoalinToolSpace(distance_to_travel_in_tool,
                                                               move_group_ptr_, listener_ptr_);
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    // create an instance HandeyeCalibration, to collect samples and coimpute calibration between
    // camera to robot end-effector
    ...
    calib_start_box.setText(
        ...
        "A SET OF RANDOM POSES WILL BE GENERATED AND AT EACH POSE AN ITERATION OF CALIBRATION WILL "
        "TAKE PLACE \n \n"
    ...
    // Each Pose variants will contin 10 poses , so the total generated poses will be
    // num_random_pose_variants * 10
    ...
    while (ros::ok() && (executed_poses_counter < num_random_pose_variants * 8))
    {
        ...
        if (is_generated_pose_planable)
        {
            ...
            ROS_INFO(
                "ROBOT RECAHED TO THE GENERATED POSE, WAITING 3 SECONDS TO LET MARKER STABILIZE "
                "... \n");
            // now sleep for 3 second
            sleep(3.0);

            //  check if marker is visiable and detected, (look at the definition of
            //  isTransformValid)
            if (handeye_node.isTransformValid())
            {
                handeye_node_ptr_->takeSample();
                handeye_node_ptr_->computeCalibration(executed_poses_counter + 1,
            ...
            else
            {
                QMessageBox Msgbox;
                Msgbox.setText(
                    "THE MARKER TRANSFROM IS OLDER THAN 1 SECOND , THIS LIKELY MEANS THAT THE "
                    "MARKER WAS NOT "
                    "DETCETED, "
        ...
        {
            ROS_ERROR(
                "RANDOMLY GENERATED POSE %d FOR CALIBRATION WAS UNREACHABLE/UNPLANABLE , WE ARE "
                "GOING TO SKIP THIS "
                "POSE!!! \n",
                executed_poses_counter);
            executed_poses_counter++;
