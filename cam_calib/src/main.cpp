...
int main(int argc, char **argv)
{
    ...
    Msgbox.setText(
        ...
        "IF THE ALGORITHM IS UNABLE TO DETECT ALL THE POINTS THAT POSE WILL BE SKIPPED AND NOT "
        "INCLUDED IN \n "
        "\n"
    ...
    while (ros::ok() && (executed_poses_counter < num_random_pose_variants * 8))
    {
        ...
        if (is_generated_pose_planable)
        {
            ...
            // here everything is fine, so take that goddamn sample , let the user select the
            // keypoints and compute calibration
